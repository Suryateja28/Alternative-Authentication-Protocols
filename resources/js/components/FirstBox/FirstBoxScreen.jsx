...
import RegisterGraph from './RegisterGraph';
import { Target, ArrowLeft } from 'lucide-react';

const FirstBoxScreen = ({ onBack }) => {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4">
                <div className="max-w-md w-full bg-slate-800/80 backdrop-blur-xl rounded-3xl p-8 border border-slate-700 shadow-2xl text-center">
                    ...
                    <p className="text-slate-400 mb-8">You have successfully authenticated using your secure graph pattern.</p>
                    <button 
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4 relative overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-md bg-slate-800/80 backdrop-blur-xl rounded-3xl p-8 border border-slate-700 shadow-2xl">
                {isLoginMode ? (
                    <LoginGraph onToggleMode={toggleMode} onLoginSuccess={handleLoginSuccess} />
                ) : (
                    <RegisterGraph onToggleMode={toggleMode} />
                )}
            </div>
        </div>
    );
